...
def draw_lines_on_image_cv(image, points, draw_action=False, num_subdivisions=100):
    ...
    for idx, (x, y) in enumerate(pixel_points):
        if draw_action:
            if idx == 0 or gripper_status[idx] != gripper_status[idx - 1]:
                circle_color = (
                    (0, 0, 255) if gripper_status[idx] == GRIPPER_CLOSE else (255, 0, 0)
                )
    ...
    colors = (cmap(np.linspace(0, 1, len(interpolated_points)))[:, :3] * 255).astype(
        np.uint8
    )
...
def process_answer(input_str):
    ...
    ).replace("<action>Open Gripper</action>", "(1001.0, 1001.0)")
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Row():
                ...
                temperature_input = gr.Slider(
                    minimum=0.0, maximum=1.0, value=0.0, label="Temperature"
                )
                top_p_input = gr.Slider(
                    minimum=0.0, maximum=1.0, value=0.95, label="Top P"
                )
                crop_type_input = gr.Radio(
                    ["Stretch", "Center Crop"],
                    value="Stretch",
        ...
        with gr.Column():
            ...
            gr.Examples(
                examples=examples, inputs=[image_input, quest_input], label="Example"
            )
